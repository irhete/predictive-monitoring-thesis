...
def create_and_evaluate_model(args):
    ...
    for cv_iter in range(n_splits):
        ...
        # read encoded data
        dt_train_prefixes = pd.read_csv(os.path.join(folds_dir, "fold%s_train.csv" % cv_iter), sep=";")
        dt_test_prefixes = pd.read_csv(os.path.join(folds_dir, "fold%s_test.csv" % cv_iter), sep=";")
        ...
        dt_train_prefixes = pd.read_csv(os.path.join(folds_dir, "fold%s_bucket_train.csv" % cv_iter), sep=";")
        dt_test_prefixes = pd.read_csv(os.path.join(folds_dir, "fold%s_bucket_test.csv" % cv_iter), sep=";")
        bucketer = BucketFactory.get_bucketer(bucket_method, **bucketer_args)
        bucket_assignments_train = bucketer.fit_predict(dt_train_prefixes, preencoded=True)
        bucket_assignments_test = bucketer.predict(dt_test_prefixes, preencoded=True)
            
        with open(os.path.join(folds_dir, "fold%s_train_y.csv" % cv_iter), "rb") as fin:
            train_y_prefixes = np.array(pickle.load(fin))
        with open(os.path.join(folds_dir, "fold%s_test_y.csv" % cv_iter), "rb") as fin:
            test_y_prefixes = np.array(pickle.load(fin))
            
        preds_all = []
        test_y_all = []
        for bucket in set(bucket_assignments_test):
            # select prefixes in current bucket
            dt_train_bucket = dt_train_prefixes[bucket_assignments_train==bucket]
            dt_test_bucket = dt_test_prefixes[bucket_assignments_test==bucket]
            ...
            cls.fit(dt_train_bucket, train_y_prefixes[bucket_assignments_train==bucket])
            preds = cls.predict_proba(dt_test_bucket)

            preds_all.extend(preds)
            test_y_all.extend(test_y_prefixes[bucket_assignments_test==bucket])
...
for dataset_name in datasets:
    ...
    for train_chunk, test_chunk in dataset_manager.get_stratified_split_generator(train, n_splits=n_splits):
        ...
        # encode data for bucketing
        bucket_encoder = EncoderFactory.get_encoder(method=bucket_encoding,
                                                    case_id_col=dataset_manager.case_id_col,
                                                    dynamic_cat_cols=[dataset_manager.activity_col],
                                                    dynamic_num_cols=[])
        
        dt_train_bucket_encoded = bucket_encoder.fit_transform(dt_train_prefixes)
        pd.DataFrame(dt_train_bucket_encoded).to_csv(os.path.join(folds_dir, "fold%s_bucket_train.csv" % cv_iter), sep=";", index=False)
        del dt_train_bucket_encoded
        
        dt_test_bucket_encoded = bucket_encoder.transform(dt_test_prefixes)
        pd.DataFrame(dt_test_bucket_encoded).to_csv(os.path.join(folds_dir, "fold%s_bucket_test.csv" % cv_iter), sep=";", index=False)
        del dt_test_bucket_encoded
        ...
        del dt_train_prefixes, dt_test_prefixes, train_y, test_y
