...
Usage:
  experiments_test_interrun_stability.py <dataset> <method> <classifier> <auc_weight> <inter-run-stability_weight>

Example:
    experiments_test_interrun_stability.py bpic2012_cancelled single_laststate xgboost 1 5
...
for dataset_name in datasets:
    ...
    if "sepsis" in dataset_name:
        detailed_results.case_id = detailed_results.case_id.str.replace("missing_caseid", "missing")
    max_underscores_in_caseid = detailed_results.case_id.apply(lambda x: len(x.split("_"))).max()
    if "single" in method_name:
        detailed_results["nr_events"] = detailed_results.case_id.apply(lambda x: 1 if len(x.split("_")) < max_underscores_in_caseid else x.split("_")[-1])
    detailed_results["case_id"] = detailed_results.case_id.apply(lambda x: x if len(x.split("_")) < max_underscores_in_caseid else x.split("_")[0])
